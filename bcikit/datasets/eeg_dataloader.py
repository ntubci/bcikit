...
class EEGDataloader(EEGDataset):
    ...
    Args:
        ...
        preprocessing_fn (function, optional): A function to perform a preprocessing on `data` and `targets`, then return `data` and `targets`. Default: ``None`` 
            Example:
                ```
                def preprocessing_fn(data, targets, **kwargs)
                    ...
                    return data, targets
                ```
        data_selection_fn (function, optional): A function that does data selection and return PyTorch dataloaders.
            Example:
                ```
