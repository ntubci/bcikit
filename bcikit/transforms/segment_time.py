...
def segment_data_time_domain(data, window_len, shift_len, sample_rate, add_segment_axis=True, verbose=False):
    """
    Divide a time domain into length of `window_len`. Function created to accept 5-dim data

    Args:
        signal : ndarray, 3 or 5 dimension, (trial,channel,time) or (subject,session,trial,channel,time)
            Input data with time domain at last's axis
        window_len : int
        ...
        add_segment_axis : boolean, default: True
            If True, segmented shape is (...,channel,#segments,time), otherwise (...,channel,time)
        verbose : boolean, default: False
            Print details
    
    Returns:
        segmented : ndarray, shape (...,channel,#segments,time) or (...,channel,time)
            Segmented matrix

    TODO: maybe can remove the 2 for-loops and make it more numpy
    """

    assert len(data.shape) == 3 or len(data.shape) == 5, "data shape must be (trial,channel,time) or (subject,session,trial,channel,time)"

    if verbose:
        print("Processing segment_data_time_domain, data shape:", data.shape)

    if len(data.shape) == 3:
        return segment_signal(data, window_len, shift_len, sample_rate, add_segment_axis, verbose)
        ...
        for sub_idx in range(data.shape[0]):
            for ses_idx in range(data.shape[1]):
                this_data = data[sub_idx, ses_idx, :, :, :]
                this_data = segment_signal(this_data, window_len, shift_len, sample_rate, add_segment_axis, verbose)

                if segmented_data is None:
                    if add_segment_axis:
                        segmented_data = np.zeros((data.shape[0], data.shape[1], data.shape[2], data.shape[3], this_data.shape[2], this_data.shape[3]))
                    else:
                        segmented_data = np.zeros((data.shape[0], data.shape[1], data.shape[2], this_data.shape[0], this_data.shape[1], this_data.shape[2]))

                this_data = np.expand_dims(this_data, axis=(0, 1))
                segmented_data[sub_idx, ses_idx, :, :, :] = this_data
...
def segment_signal(signal, window_len, shift_len, sample_rate, add_segment_axis=True, verbose=False):
    r"""
    Divide a time domain into length of `window_len`.
    ...
    signal_overlap = (window_len*sample_rate-shift_len)

    num_segments = int(
        np.ceil((signal.shape[2]-signal_overlap)/(duration-signal_overlap)))
        ...
        for trial in range(0, signal.shape[0]):
            for channel in range(0, signal.shape[1]):
                segmented[trial, channel, :, :] = buffer(
                    signal[trial, channel], duration, signal_overlap, num_segments)
    ...
    if verbose:
        ...
        print('Overlap', signal_overlap)
        print('#segments', num_segments)
        print('Shape from',  signal[0, 0].shape, 'to', buffer(
            signal[0, 0], duration, signal_overlap, num_segments).shape)
