...
from bcikit.transforms.fft import fast_fourier_transform
...
def butter_bandpass_filter(signal, lowcut, highcut, sample_rate, order, verbose=False):
    ...
    axis = 4 if len(signal.shape)==5 else 3
    y = sosfiltfilt(sos, signal, axis=axis)

    if verbose and len(signal.shape)==3:
        tmp_x = signal[0, 0]
        tmp_y = y[0, 0]
